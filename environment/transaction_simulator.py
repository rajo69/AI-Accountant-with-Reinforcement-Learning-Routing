...
Workflow
--------
1. Load 50 seed transactions from data/seeds/transactions.json
...
FIXTURES_DIR = PROJECT_ROOT / "data" / "seeds"
...
the RL routing policy. It is derived from the 50-transaction seed set in
`data/seeds/transactions.json` (originally sourced from the parent AI Accountant
project's eval fixtures).
    ...
    def _load_seeds(self) -> list[RawTransaction]:
        ...
        if not path.exists():
            raise FileNotFoundError(
                ...
                "Ensure data/seeds/transactions.json exists."
            )
